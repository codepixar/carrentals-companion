...
function carrentals_import_files() {
	...
	$demoImg = '<img src="'.plugins_url( 'screen-image.png', __FILE__ ) .'" alt="'.esc_attr__( 'Demo Preview Imgae', 'carrentals-companion' ).'" />';
  ...
  return array(
    array(
      ...
      'import_customizer_file_url'   => plugins_url( 'carrentals-customizer.dat', __FILE__ ),
...
function carrentals_import_plugin_page_setup( $default_settings ) {
	...
	$default_settings['page_title']  = esc_html__( 'One Click Demo Import' , 'carrentals-companion' );
	$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'carrentals-companion' );
...
function carrentals_demo_import_custom_scripts(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'carrentals-demo-import' ){
		// style
		wp_enqueue_style( 'carrentals-demo-import', plugins_url( 'css/demo-import.css', __FILE__ ), array(), '1.0', false );
	}
