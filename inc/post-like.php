...
function carrentals_sl_enqueue_scripts() {
	...
	wp_enqueue_script( 'carrentals-post-likes', plugins_url( '../js/post-likes.js', __FILE__ ), array( 'jquery' ), '0.5', false );

	wp_localize_script( 'carrentals-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'carrentals-core' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_carrentals_process_simple_like', 'carrentals_process_simple_like' );
add_action( 'wp_ajax_carrentals_process_simple_like', 'carrentals_process_simple_like' );
function carrentals_process_simple_like() {
	// CarRentals
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'carrentals-core' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // CarRentals
	if ( $is_comment == 1 ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'carrentals-core' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'carrentals-core' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'carrentals-core' );
...
// User Profile List
add_action( 'show_user_profile', 'carrentals_show_user_likes' );
add_action( 'edit_user_profile', 'carrentals_show_user_likes' );
function carrentals_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'carrentals-core' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'carrentals-core' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
